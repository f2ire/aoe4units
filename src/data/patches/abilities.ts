...
export const techAbilityInteractions: TechAbilityInteraction[] = [
  ...
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'spearman', apply: (s) => ({ ...s, attackSpeed: 1.30 }) },
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'kanabo-samurai', apply: (s) => ({ ...s, attackSpeed: 1.00 }) },
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'naginata-samurai', apply: (s) => ({ ...s, attackSpeed: 1.00 }) },
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'ozutsu', apply: (s) => ({ ...s, attackSpeed: 3.71 }) },
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'yumi-ashigaru', apply: (s) => ({ ...s, attackSpeed: 1.30 }) },
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'tanegashima-ashigaru', apply: (s) => ({ ...s, attackSpeed: 1.95 }) },
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'mounted-samurai', apply: (s) => ({ ...s, attackSpeed: 1.15 }) },
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'yari-cavalry', apply: (s) => ({ ...s, attackSpeed: 1.21 }) },
  { requiredTech: 'sword-hunt-statue-age-up', requiredAbility: 'ability-daimyo-aura-4', unitId: 'ikko-ikki-monk', apply: (s) => ({ ...s, attackSpeed: 1.00 }) },
...
function createTeutonicWrath(): Ability {
  return {
    ...
    minAge: 4,
    active: 'always',
    icon: '/abilities/TeutonicWrath.png',
...
function createBattleGlory(): Ability {
  return {
    ...
    minAge: 4,
    active: 'manual',
    icon: '/abilities/battleglory.png',
    description: 'Teutonic Knight reduces the opponent\'s melee and ranged armor by 2.',
    unique: true,
...
function createKnightlyBrotherhood(): Ability {
  return {
    ...
    minAge: 2,
    active: 'manual',
    icon: '/abilities/Knightly_brotherhood.png',
    description: "This unit gains +1 max health for each of the player's heavy cavalry units within a 5 tile radius from it.",
    unique: true,
    counterMax: 200,
    counterHideMax: true,
    counterStep: 1,
...
function createBludgeoningAttacks(): Ability {
  return {
    ...
    minAge: 4,
    active: 'always',
    icon: '/abilities/Bludgeoning-attacks.png',
...
function createHarborAura(): Ability {
  return {
    ...
    minAge: 1,
    active: 'manual',
    icon: '/abilities/templar_harbor.png',
...
function createStrongerTogether(): Ability {
  return {
    ...
    minAge: 1,
    active: 'manual',
    icon: '/abilities/Stronger_Together.png',
...
function createRiddariThrownAxes(): Ability {
  return {
    ...
    displayClasses: [], classes: [],
    minAge: 2, active: 'manual',
    icon: '/abilities/Thrown_Axes.png',
...
function createRiddariChampionAura(): Ability {
  return {
    ...
    displayClasses: [], classes: [],
    minAge: 2, active: 'manual',
    icon: '/abilities/Riddari_Champion_Aura.png',
...
function createHorsemanChampionAura(): Ability {
  return {
    ...
    displayClasses: [], classes: [],
    minAge: 2, active: 'manual',
    icon: '/abilities/Horseman_Champion_Aura.png',
...
function createUnwaveringFocus(): Ability {
  return {
    ...
    displayClasses: [], classes: [],
    minAge: 2, active: 'manual',
    icon: '/abilities/Unwavering_Focus_AoE4.png',
...
// One ability per age (Khan War Cry pattern) so each age shows its own icon + tooltip.
// Effective multiplier per unit = p4 × 1.2 / (base × factor); factor 1.1/1.15/1.2 = age 2/3/4.
function createDaimyoAura(age: 2 | 3 | 4, factor: number, uiTooltip: string): Ability {
  const m = (p4: number, base: number) => p4 * 1.2 / (base * factor);
  const e = (id: string, p4: number, base: number) => ({ property: 'attackSpeed', select: { id: [id] }, effect: 'multiply', value: m(p4, base), type: 'ability' as const });
  return {
    id: `ability-daimyo-aura-${age}`,
    name: 'Daimyo Aura',
    type: 'ability',
    civs: ['sen'],
    displayClasses: [],
    ...
    description: `The Daimyo grants all human military units within a 6-tile radius +${5 + (age - 1) * 5}% melee, ranged, and torch attack speed.`,
    unique: false,
    uiTooltip,
    effects: [],
    variations: [{
      id: `ability-daimyo-aura-${age}-v`,
      baseId: `ability-daimyo-aura-${age}`,
      type: 'ability',
      name: 'Daimyo Aura',
      pbgid: 999610 + (age - 2),
      attribName: `ability_daimyo_aura_${age}`,
      age,
      civs: ['sen'],
      classes: [], displayClasses: [], unique: false,
      costs: { food: 0, wood: 0, stone: 0, gold: 0, vizier: 0, oliveoil: 0, total: 0, popcap: 0, time: 0 },
      producedBy: [],
      effects: [
        e('spearman', 1.62, 1.875),
        e('kanabo-samurai', 1.12, 1.375),
        e('naginata-samurai', 1.12, 1.375),
        e('ozutsu', 4.03, 4.750),
        e('yumi-ashigaru', 1.37, 1.625),
        e('tanegashima-ashigaru', 2.10, 2.500),
        e('mounted-samurai', 1.23, 1.500),
        e('yari-cavalry', 1.56, 1.750),
        e('ikko-ikki-monk', 1.12, 1.375),
      ],
    }],
    shared: {}
  } as Ability;
...
function createTughlaqabadProtector(): Ability {
  return {
    ...
    displayClasses: [], classes: [],
    minAge: 2, active: 'manual',
    icon: '/abilities/Tughlaqabad-Protector.png',
...
function createGovernorOfJalor(): Ability {
  return {
    ...
    classes: [],
    minAge: 2,
    icon: '/abilities/Governor_of_Jalor.png',
...
function createElephantFormations(): Ability {
  return {
    ...
    minAge: 2,
    active: 'always',
    icon: '/abilities/Elephant_Formations.png',
...
function createTrampleTug(): Ability {
  return {
    ...
    minAge: 2,
    active: 'manual',
    icon: '/abilities/trample_tug.png',
...
export function applyAbilityPatches(abilities: Ability[]): Ability[] {
  ...
  const allAbilitiesMerged = [
    ...
    createDaimyoAura(2, 1.1, '+10% AS announced. Effective avg: +9.4% (spread: +2.8% yari-cavalry → +12.5% kanabo/naginata/ikko-ikki).'),
    createDaimyoAura(3, 1.15, '+15% AS announced. Effective avg: +14.3% (spread: +7.5% yari-cavalry → +17.7% kanabo/naginata/ikko-ikki).'),
    createDaimyoAura(4, 1.2, '+20% AS announced. Effective avg: +19.3% (spread: +12.2% yari-cavalry → +22.8% kanabo/naginata/ikko-ikki).'),
